...
from configs import CLR_ERROR, CLR_SUCCESS, LOGS_CHANNEL
...
class Events(commands.Cog):
    ...
    async def on_command(self, ctx: commands.Context):
        ...
        embed = disnake.Embed(
            title=f"Executed command '{command_name}'", color=CLR_SUCCESS
        )
        embed.add_field(
            name=f"Executed by {ctx.author} (ID: {ctx.author.id})",
            value=f"in {ctx.guild.name} (ID: {ctx.guild.id})",
        )
        embed.set_footer(text=f"On {current_date} at {current_time}")
        try:
            print(
                f"Executed command '{command_name}' in {ctx.guild.name} (ID: {ctx.guild.id})\nBy {ctx.author} (ID: {ctx.author.id})\nOn {current_date} at {current_time}"
            )
            # logs channel
    ...
    async def on_slash_command(self, inter):
        ...
        command_name = str(split[0])
        embed = disnake.Embed(
            title=f"Executed slash command '{command_name}'", color=CLR_SUCCESS
        )
        embed.add_field(
        ...
        try:
            print(
                f"Executed slash command '{command_name}' in {inter.guild.name} (ID: {inter.guild.id})\nBy {inter.author} (ID: {inter.author.id})\nOn {current_date} at {current_time}"
            )
            # logs channel
            channel = self.bot.get_channel(LOGS_CHANNEL)
            await channel.send(embed=embed)
    ...
    async def on_user_command(self, inter):
        ...
        command_name = str(split[0])
        embed = disnake.Embed(
            title=f"Executed user command '{command_name}'", color=CLR_SUCCESS
        )
        embed.add_field(
            name=f"Executed by {inter.author} (ID: {inter.author.id})",
            value=f"in {inter.guild.name} (ID: {inter.guild.id})",
        ...
        try:
            print(
                f"Executed user command '{command_name}' in {inter.guild.name} (ID: {inter.guild.id})\nBy {inter.author} (ID: {inter.author.id})\nOn {current_date} at {current_time}"
            )
            # logs channel
            channel = self.bot.get_channel(LOGS_CHANNEL)
            await channel.send(embed=embed)
    ...
    async def on_slash_command_error(
        self, inter: disnake.AppCmdInter, error: commands.CommandError
    ):
        embed = disnake.Embed(
