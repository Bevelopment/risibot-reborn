...
class Moderation(commands.Cog):
    ...
    @slash_command(name="nick", description="Change the nickname of a user")
    ...
    @slash_command(name="kick", description="Kick a member from a server")
    ...
    @slash_command(
        name="ban", description="Ban a user from a server and all messages after days"
    )
    async def ban(
        self, inter, member: disnake.Member, delete_msg_days: int, *, reason: str = None
    ):
    ...
    @slash_command(name="unban", description="Unban a user, use the user's id")
    ...
    @commands.has_permissions(administrator=True)
    @slash_command(name="move", description="Move the member to a voice channel")
    async def move(
        self, inter, member: disnake.User, channel: disnake.VoiceChannel = None
    ):
        if member.voice:
    ...
    @commands.has_permissions(manage_channels=True)
    @slash_command(
        name="archive", description="Archive a channel, voice channel or stage channel"
    )
    async def archive(
        self,
        inter,
        channel: disnake.TextChannel = None,
        voice_channel: disnake.VoiceChannel = None,
        stage_channel: disnake.StageChannel = None,
    ):
